...
const OKRsPage = {
    ...
    async render() {
        ...
        // Suporta múltiplos departamentos (para verificação de permissões)
        ...
        // Mapa de mini_cycle_id -> cycle.ativo (usa getAll p/ incluir ciclos inativos)
        ...
        content.innerHTML = `
            <div class="page-header" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;">
                <div>
                    <h2 style="font-size:20px;font-weight:700;color:var(--top-blue);margin-bottom:4px;">Todos os OKRs</h2>
                    <p style="color:var(--text-muted);font-size:13px;">${okrs.length} ${okrs.length === 1 ? 'OKR cadastrado' : "OKR's cadastrados"}</p>
                </div>
    ...
    async renderList() {
        ...
        const isConsultor = currentUser && currentUser.tipo === 'consultor';
        const userDepartmentNames = this.getUserDepartmentNames(currentUser);
        // Na página "Todos os OKRs", somente admin pode editar OKRs e KRs
        ...
        // Filtro por departamento (disponível para todos)
                                                                            ...
                                                                            : `<div class="kr-evidence-file">
                                                                                ...
                                                                                <div class="file-info">
                                                                                    ${(ev.name || ev.content || '').toLowerCase().endsWith('.html')
                                                                                        ? `<a href="#" onclick="event.preventDefault(); OKRsPage.openHtmlEvidence('${convertToProxyUrl(ev.content)}')" class="file-download-link">
                                                                                            ${ev.name || 'Arquivo'}
                                                                                        </a>`
                                                                                            ...
                                                                                            ${init.evidence.map((ev, idx) => `
                                                                                                <div class="init-evidence-item">
                                                                                                    ${ev.type === 'text'
                                                                                                        ...
                                                                                                        : (ev.name || ev.content || '').toLowerCase().endsWith('.html')
                                                                                                            ? `<a href="#" onclick="event.preventDefault(); OKRsPage.openHtmlEvidence('${convertToProxyUrl(ev.content)}')" class="init-evidence-file">
                                                                                                                <svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    addInitEvidenceField() {
        ...
        item.innerHTML = `
            ...
            ${type === 'text'
                ? `<textarea class="form-control evidence-content" rows="2" placeholder="Descrição da evidência..."></textarea>`
                : `<div class="file-upload-wrapper">
                    <input type="file" id="init-file-input-${index}" class="evidence-file-input" accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg,.gif,.html" onchange="OKRsPage.handleInitFileSelect(${index}, this)">
